...
## 🔧 Design Notes / Governance Considerations
This workflow shows how low-code/no-code platforms like KNIME can tap into the power of LLMs to dynamically generate and execute Cypher queries. While powerful, this also raises critical security and governance considerations.

Unlike some developer frameworks like LangChain — which explicitly offer flags like <i>allow_dangerous_requests=False</i> to block write operations — as far as I know, KNIME does not provide a built-in safeguard against potentially destructive queries (e.g., DELETE, MERGE, or SET) generated by the LLM.

This means:
<ul>
<li>Query validation is your responsibility.
<li>The LLM can return any syntactically valid Cypher query—including harmful ones—if not carefully prompted or reviewed.
</ul>
🛡️ Mitigation Strategy
<p>
To reduce risk, it's strongly recommended to connect to Neo4j using a read-only user account with the reader role. This ensures:
<ul>
<li>Only MATCH and read operations can be executed.
<li>Any LLM-generated write/delete queries will be rejected by the database engine.
<li>A safer environment for experimentation and demonstration.
</ul>
